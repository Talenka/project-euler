...
function problem11()
{
  ...
  for (i = 0; i < 20; i++)
    for (j = 0; j < 20; j++) {
      ...
      if (i < 16)
        MaxP = Math.max(MaxP, grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j]);

      if (j < 16)
        MaxP = Math.max(MaxP, grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3]);

      if (i < 16 && j < 16)
        MaxP = Math.max(MaxP, grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3]);

      if (i > 4 && j < 16)
        MaxP = Math.max(MaxP, grid[i][j] * grid[i - 1][j + 1] * grid[i - 2][j + 2] * grid[i - 3][j + 3]);
    }

	return MaxP; // --> 70600674
}
